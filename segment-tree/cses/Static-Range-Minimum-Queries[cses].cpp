...
int main() {
    ...
    vector< long long >tree(n + n, (long long)2e9);
    ...
    for(int i = n + n - 1; i > 1; i--) tree[i / 2] = min(tree[i / 2], tree[i]);
    ...
    (int node, int st, int ed, int a, int b) {
        ...
        if(b < st or ed < a) return (long long)2e9;
        ...
        return min(query(node * 2, st, mid, a, b), query(node * 2 + 1, mid + 1, ed, a, b));
